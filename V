...
function Library:open()
    ...
    if UserInputService.TouchEnabled then
        containerSize = UDim2.new(0.7, 0, 0.9, 0)
    else
...
function Library:new()
    ...
    Shadow.Size =  UDim2.new(0.71, 0, 0.91, 0)
    else
        Shadow.Position = UDim2.new(0.5, 0, 0.5, 0)
   ...
   Top.Size = UDim2.new(1, 0, 0.08, 0) 
    ...
    TextLabel.Text = getgenv().namehub or "getgenv().namehub"
...
function Module:create_multidropdown()
    ...
    Dropdown.placeholders = self.placeholders or {}
    ...
    if pos.X >= boxPos.X and pos.X <= (boxPos.X + boxSize.X) and pos.Y >= boxPos.Y and pos.Y <= (boxPos.Y + boxSize.Y) then
        open = not open
        if open then
            Dropdown:open()
        else
    ...
    function Dropdown:SetOption(newOptions, selectedValues)
        ...
        for _, option in ipairs(self.options) do
            ...
            if self.placeholders and type(self.placeholders) == "table" then
                for _, placeholder in ipairs(self.placeholders) do
                    if option == placeholder then
                        isPlaceholder = true
                        break
                    end
                end
...
function Module.create_dropdown(self)
    ...
    function Dropdown:SetOption(newOptions, selectedValue)
        if type(newOptions) == "table" then
            self.options = newOptions
        end
        
        if selectedValue and table.find(self.options, selectedValue) then
            self.selectedOption = selectedValue
            TextLabel.Text = selectedValue
            Library.Flags[self.flag] = selectedValue
            self.callback(selectedValue)
        elseif selectedValue then
            warn("Selected value '" .. tostring(selectedValue) .. "' not found in options")
        end
